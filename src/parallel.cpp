...
class Knapsack {
   ...
   public:
    ...
    auto recursion(const size_t &start, const size_t &end, const int divisions,
                   const int depth = 0) {
        if (depth == divisions) {
    ...
    /**
     * Call recursion with a maximum depth of 1
     */
    void parallelNemhauserUllmann() { printPoints(recursion(0, m_size, 1)); }
